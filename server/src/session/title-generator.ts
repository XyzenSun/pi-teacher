// 移植自 pi-web v0.9.0 lib/session-title.ts（MIT License）。
// 差异：pi-teacher 把结果写回 pi_session.name（pi-web 只返回给调用方）；
// 时机不变——轮次空闲（onAgentRunComplete）后端独立发起，不进会话上下文。
import {
  Agent,
  type AgentMessage,
  type AgentOptions,
...
import type { AgentSessionLike } from "../bridge/pi-types.ts";
 ...
 */
function buildSessionTitleAgentOptions(source: Agent): AgentOptions {
 ...
 */
function appendTitleRequestToTrailingUser(messages: AgentMessage[]): AgentMessage[] {
...
function getAssistantResult(agent: Agent, historyLength: number): GeneratedSessionTitle {
  ...
  for (let i = generatedMessages.length - 1; i >= 0; i--) {
    ...
    const text = message.content
      ...
      .trim();
    if (!text) continue;
    return { title: parseGeneratedSessionTitle(text) };
  }
...
/** 从一个活动会话生成标题（独立 LLM 调用，工具全部替换成抛错影子）。 */
export async function generateSessionTitle(source: AgentSessionLike): Promise<GeneratedSessionTitle> {
  const sourceAgent = source.agent as unknown as Agent;
