...
> 状态：已实现、完成工程验收并部署到隔离验证容器（2026-09-11，commit `97dd87c`），WebUI 待用户验收。
...
| Docker / `verify:remote` | 重建 `pi-teacher:local` 后按 `deploy.md` 停止 → 备份 → `up -d` 升级隔离验证容器：启动日志无错误，升级前备份库仍含 `card.source_essence_path`、升级后已删列，账号 / 会话 / `user_env` 保留；`verify:remote` 49 通过、0 失败，新建学习会话真实生成空 `essence/` |
| 浏览器 | 未执行，由用户验收 |
...
既有 `~/pi-teacher/AGENTS.md` 需从设置页手工合并新规范，程序不覆盖。原三段自定义文案无需重置，精华段缺行即默认生效。容器重建镜像即可获得新逻辑，不需要重建数据库或学习会话；删列不可由旧镜像自动逆转，回退前先恢复升级前备份。隔离验证容器已按此流程升级并通过 `verify:remote`。详见 `docs/deploy.md`。
