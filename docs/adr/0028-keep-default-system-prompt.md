...
1. Available tools 段——每个工具的描述由 `toolSnippets` 动态注入，我们的 16 个扩展工具（`defineTool` 的 `promptSnippet`）也走这条路
...
- **「提示词都在库里、WebUI 可编辑」是既定规则**（`数据库设计.md`）。系统提示词定制是代码级配置，把教学角色放进它，就等于一部分角色行为脱离了模板库的增删改切换体系。
