...
学习助手需要 Agent Loop、流式对话、tool calling 和自动上下文压缩，这些都是已被反复实现过的底层能力，自研会挤占学习业务本身的开发精力。我们选择 `earendil-works/pi`（Pi Agent Harness）作为执行器，不再研究 DeepSeek Harness，也不自己重写上述能力。
...
- Pi 处于 0.x 阶段且迭代很快，需要锁定精确版本，并把嵌入点收敛到少量适配代码中。
