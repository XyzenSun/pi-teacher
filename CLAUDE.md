...
pi-teacher/
  ...
  skills/                       ← 内置 skill（tavily-search / exa-search / pullpage / sbx，ADR-0040 / 0041）：COPY 进镜像，entrypoint 每次启动覆盖进 ~/.pi/agent/skills 同名目录
```
...
- 内置 skill（ADR-0040 / 0041）：四个目录随镜像走，目录名即各自 `SKILL.md` 的 `name`；`tavily-search` / `pullpage` / `exa-search` 是零依赖 Node 脚本（脚本本身即源码，改行为直接改它），`sbx` 是 esbuild bundle（重建方法见 `skills/sbx/sourcecode/README.md`）。仓库不再有 Go。「何时用哪个 skill」只写在各自 `description` 里，Pi 会自动注入 `<available_skills>`，**不写进全局 AGENTS.md**；`allowed-tools` 字段 Pi v0.84.2 不解析。
