...
```
pi-teacher/
  AGENTS.md                     ← 本文件（pi 项目上下文）；根目录只放它，其余文档一律进 docs/
...
| `npm run verify:catalog` | 模型目录单例的界面外修改感知（指纹自动重载）与 /api/models 默认模型降级 |
...
- 模型目录（`session/models.ts`）是进程级单例，按 models.json + auth.json 的 mtime/size 指纹检测界面外修改（pi CLI、手动编辑、版本迁移）并自动 refresh，改完无需重启后端；默认模型每次请求现读 settings.json，不进快照。`GET /api/models` 在默认模型不可用时降级为列表照常返回 + `defaultModel: null`，开会话路径仍抛 503（不静默换模型）。
...
| 疑问某个约定为何如此 | `docs/adr/` 对应编号；0027（不用 MCP）、0028（不替换默认 system prompt）、0030（工作目录归属）、0031（风格切换）、0033（提示词四层来源）、0034（用户环境变量）、0035–0038（助教常驻与清除、维护提醒、真删除、`files/`）、0039（资料归档与学习精华、卡片解耦）、0040–0042（内置 skill：接入与沙箱载体、tavily Node 重写、tingwu-transcribe）最常用 |
